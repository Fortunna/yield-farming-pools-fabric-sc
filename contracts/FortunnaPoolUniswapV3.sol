...
contract FortunnaPoolUniswapV3 is Initializable, Pausable, ReentrancyGuard, IFortunnaPool, IERC721Receiver {
    struct RewardInfo {
        ...
        updateReward(_msgSender()) 
    {
        ...
        if (totalLiquidity == 0) {
            (addedLiquidity, addedActualAmounts[0], addedActualAmounts[1]) = 
        ...
        updateReward(_msgSender()) 
    {
        ...
        require(
            amount > 0, 
            "FortunnaPoolUniswapV3: can not withdraw 0"
        );
        (uint256 withdrawn0, uint256 withdrawn1) = _decreaseLiquidity(sender, amount);
    ...
    function getReward() public whenNotPaused nonReentrant updateReward(_msgSender()) {
        address sender = _msgSender();
        ...
        onlyRole(DEFAULT_ADMIN_ROLE) 
    {
        ...
        for (uint8 i = 0; i < ASSETS_COUNT; i++) {
            if (block.timestamp >= periodFinish) {
                ...
                rewardRates[i] = (totalRewards[i] + leftover) / REWARDS_DURATION;
            }
    ...
    function earned(address user, uint8 index) public view returns (uint256) {
        return
            ...
            * (rewardPerToken(index) - rewardsInfo[user].userRewardsPerTokensPaid[index])
            / FortunnaLib.PRECISION
    ...
    function _mintNewPosition(address sender, uint256 amount0ToMint, uint256 amount1ToMint)
        internal
        returns (
            uint128 liquidity,
            uint256 amount0,
        ...
        )
    {
        ...
        TransferHelper.safeTransferFrom(tokens[0], sender, address(this), amount0ToMint);
        TransferHelper.safeTransferFrom(tokens[1], sender, address(this), amount1ToMint);

        // Approve the position manager
        TransferHelper.safeApprove(tokens[0], address(nonfungiblePositionManager), amount0ToMint);
        TransferHelper.safeApprove(tokens[1], address(nonfungiblePositionManager), amount1ToMint);
        ...
        if (amount0 < amount0ToMint) {
            TransferHelper.safeApprove(tokens[0], address(nonfungiblePositionManager), 0);
            uint256 refund0 = amount0ToMint - amount0;
            TransferHelper.safeTransfer(tokens[0], sender, refund0);
        }

        if (amount1 < amount1ToMint) {
            TransferHelper.safeApprove(tokens[1], address(nonfungiblePositionManager), 0);
    ...
    function _decreaseLiquidity(address sender, uint128 amountToDecrease) internal returns (uint256 amount0, uint256 amount1) {
    ...
    {
        TransferHelper.safeTransferFrom(tokens[0], sender, address(this), amountAdd0);
        TransferHelper.safeTransferFrom(tokens[1], sender, address(this), amountAdd1);

        TransferHelper.safeApprove(tokens[0], address(nonfungiblePositionManager), amountAdd0);
        TransferHelper.safeApprove(tokens[1], address(nonfungiblePositionManager), amountAdd1);
    ...
    modifier updateReward(address user) {
        ...
        for (uint8 i = 0; i < ASSETS_COUNT; i++) {
            ...
            if (user != address(0)) {
                ...
                rewardsInfo[user].userRewardsPerTokensPaid[i] = rewardsPerTokenStored[i];
            }
        }
        _;
    }
    ...
    modifier onlyInitializing {
        require(!isInitialized, "FortunnaPoolUniswapV3: cannot call the function when the contract already initialized.");
        _;
    }
